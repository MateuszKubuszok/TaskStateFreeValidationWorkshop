...
import io.scalawave.workshop.free.CatsCalculation.Ops
...
class CatsCalculationStateInterpreterSpec extends Specification with CatsInterpreterTestParams {
  ...
  "CatsCalculationStateInterpreter" should {
    ...
    "display value" in new Fixture {
      ...
      // when
      program.foldMap(interpreter).run(initialConfig).value._2
